...
- `.agents/skills/` 是本项目自带 skill 的唯一仓库位置:`reorient`、`playtest-protocol`、`tune-params`、`research-decision`、`portfolio-capture`、`blind-playtest`(零背景玩家,只碰仓库外成品 HTML)、`repo-audit`(只读通审,只举手不修)。`.cursor/skills/` 不保留同名副本。`evidence-before-done` 与 `learning-handoff` 是全局 skill,不在仓库内,换机器会失效。
- 调用记法:Cursor 用 `/skill-name`;Codex 用 `$skill-name`。Cursor 也会读取用户目录中的旧体系,所以旧体系仍可能被看见,靠下一条停用清单约束而非删文件。
