...
最后复查：2026-08-22（失败原型已删除；项目级 UI／UX Skills 已安装并核验，当前进入渐进知识地图语义对齐）
...
| 玩家知识图 | 显示已发现事实、允许知道的关系、当前判断与开放问题，承担外部记忆 | **Interaction direction reopened / Map semantics alignment**；DEC-025 的玩家责任继续有效；文字／面板主导原型已被用户否决并删除，只保留轻量失败记录。当前候选方向是从混沌／黑暗逐步显影的可视化进度地图；其地理、披露、前沿、路径历史和器物交互仍待逐项对齐 |
| 概率与行动层 | 真相包后验、阶段主张、价值分布、信息价值、调查机会／检测费用、继续与停止 | **Design / Experimental**；职责与依赖政策已定，具体阈值／摘要仅为 DEC-018 待验证默认 |
| 玩家知识地图交互 | 让玩家看见自己点亮了什么、证据怎样连接或冲突、哪里仍黑、哪些探索前沿因本局信息自然显现；文字只作就地解释，不替代空间主结构 | **Design frontier reopened / No approved replacement yet**；旧更新流、三域台、摘要＋关系图曾证明共享状态线路可运行，但已被用户否决并删除。项目级 UX／HCI 与审美 Skills 已固定来源安装并核验；当前只逐项讨论地图语义，不制作新版界面。地图不得泄露作者隐藏槽、唯一路线或伪装成图形化 TODO list |
