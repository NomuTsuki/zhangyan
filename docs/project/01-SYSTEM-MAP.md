...
| 游戏规则核心 | `WorldState`、合法行动、调查预算、四层信息、NPC、动态定价、交易与结算 | 独立调查/议价资源、具体证据公开、动态议价与等级制结算已经实现；最新完整结果以 `02-CURRENT-STATE.md` 为准 |
...
现有参数仍未证明平衡或有趣；TypeScript 规则、实验台和历史单文件也尚未统一为单一数值权威源。这些结构性问题已经批准作为 V2 第一项核心产品工程，不混入 V1 教师封存。
...
→ 玩家继续调查，或开始普通报价；首次正式报价后调查锁定并转入独立议价容量
→ NPC 接受 / 还价 / 拒绝 / 离场；玩家始终可以接受现价或拒绝
→ 客观交易结果 + 仅依据可见信息的判断质量
