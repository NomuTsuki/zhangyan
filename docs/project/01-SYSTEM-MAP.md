# System Map

> 描述项目当前的整体理解、职责边界和关键流动，不展开逐文件施工细节。

最后复查：2026-07-31

## 主要组成部分

| 组成部分 | 职责 | 当前状态 |
|---|---|---|
| 玩家 UI | 手机竖屏调查、证据簿、NPC 交流、证据公开、交易与复盘 | 低保真 React/单文件循环已存在；“当代雅集式”独立高保真教师演示 HTML 待制作 |
| 游戏规则核心 | `WorldState`、合法行动、调查预算、四层信息、NPC、动态定价、交易与结算 | 多轮规则切片已实现并通过 `34/34` 技术回归；结算语义和正式议价资源正在重新对齐 |
| 案件内容 | 隐藏物品真相、NPC 认知档案、观察点、陈述信号、Storylet | 漆木首饰盒首案含 3 个规则真相变体；教师演示默认旧胎重修真品 |
| NPC 纺锤决策 | 状态 delta、候选生成、硬过滤、分项评分、seed 近分扰动与回应 | 每轮保存完整可复现轨迹，候选不会无限发散 |
| 调试与可解释性 | 双方精确后验、证据可见性、四状态、回合公式、定价分项、局末状态图与双层评分 | 宽屏侧栏完整显示；手机玩家界面只显示公开因果；定价秘密仅局末解锁 |
| 构建与交付 | 生成可离线演示并最终可通过微信内网址打开的普通 H5 | 自包含 HTML 已生成；普通网页 H5 路线已确认，公开托管与微信内实机仍待后续验证 |

## 当前实现与目标规则的已知偏差

当前代码仍然代表上一轮工作原型，不应被误读为已经批准的正式数值系统：

- 正式报价和无条件买断仍消耗调查行动点；用户已经明确要求改为独立议价资源；
- 客观分仍以路径内参考线和后悔值为核心；用户已经指出它不能代表实际经营与压价结果；
- “完整说明事实 / 只强调不利部分”仍是抽象按钮，两者在首案主路径中缺少可感知差异；
- 无条件买断在当前 UI 和实验台中过于突出；风险转移应降为特定条件下的可选措施；
- `34/34` 测试表示代码对当前规则保持一致，不表示上述设计已经正确或获得认可。

规则方向已经通过 [CH-002](challenges/resolved/CH-002-settlement-bargaining-and-disclosure-alignment.md) 对齐；当前偏差等待下一版测试与实现修正。

## 当前已实现的关键流程或信息流（含已知偏差）

```text
固定物品真相 + 玩家初始先验 + 独立 NPC 私有认知 + seed
→ 玩家在共享预算内私下观察 / 开放询问 / 带证据询问 / 共同检测 / 报价
→ 私有证据只更新玩家；被公开或共同检测的证据进入共享集合
→ 双方按各自信息更新后验，NPC 四状态更新谈判档位
→ NPC 候选行为发散、过滤、评分并收敛为回应或正式重估
→ 玩家继续调查，或执行普通报价 / 无条件买断 / 接受现价 / 拒绝
→ 客观交易结果 + 仅依据可见信息的判断质量
→ 局末因果时间线、状态变化图、公式与完整回放
```

证据簿是只读信息仓库；进入证据簿不会提供行动入口，返回原页面后再由玩家决定下一步。

## 边界与依赖

- 物品客观真相决定观察结果和最终真实价值；NPC 回应与底价只读取独立认知档案和当前状态，不能偷看物品真相；
- 判断质量只读取玩家可见物证、检测和陈述信号；客观得失才读取隐藏真相；
- 同一次 NPC 回应生成的结构化信号与展示用陈述卡只计权一次；
- seed 只影响允许的幸运证据和近分候选扰动，不改变隐藏真相、固定成本或公式；
- 最终入口已经确认为通过微信聊天或公众号链接打开的普通网页 H5，不是微信小游戏；标准 Web 是当前批准路线，Unity/团结引擎不是必经步骤。
- 器物视觉使用二维插画建立位置与氛围，细纹、标记、锁扣等鉴定事实以文字证据为准；当前低价值首案不制作结算后的写实讲解图。

### 已接入玩家规则核心的双边信息边界

- 现有真相隔离已经扩展为物品真相、玩家私有证据/后验、NPC 私有认知/后验和共享证据四层，见 [DEC-007](decisions/DEC-007-dual-belief-and-strategic-disclosure.md)、[EXP-007](evidence/EXP-007-dual-posterior-disclosure-lab.md) 与 [EXP-008](evidence/EXP-008-dual-posterior-player-loop.md)；
- 玩家私有证据只有在用于质询、对峙或共同检测后才进入共享信息；共享后双方按各自专业度重新解释；
- NPC 允许真诚误记或误判，但不能故意生成与自身已知信息相反的事实陈述；
- 共享证据负责更新 NPC 主观估值，四状态主要负责改变谈判档位、让步和接受条件；
- 初始开价不再承担唯一价格锚点职责；玩家谨慎参考只读取玩家可见后验，NPC 定价只读取 NPC 可见信息与人物参数；
- 普通报价使用动态接受线；无条件买断使用独立买断线，并在接受或拒绝后终局，以交换“更低确定价”和“不能继续附加条件”的风险归属；
- 正式重估由共享证据、后验跨档或谈判档位变化触发，不能由 UI 直接指定；当前参数是否健康仍由 [HYP-004](hypotheses/HYP-004-dual-posterior-disclosure.md) 验证。

## 关键接口或交接点

- `PlayerAction` 是 UI 与规则核心之间的唯一行动输入，覆盖观察、询问、披露方式、共同检测、普通报价、无条件买断和拒绝；
- `createInitialWorldState → resolveTurn / replayActions → WorldState + Settlement` 支持无 UI 测试和完整回放；
- 案件内容通过 `CaseDefinition` 配置进入规则层；
- 玩家 UI 与开发调试表面分离：手机画布只显示玩家应知信息，精确系统过程位于桌面侧栏；
- 买下与拒绝是零行动点终局动作，保证共享预算耗尽后仍能完成一局。

## 容易耦合或失控的区域

- 将 NPC 回应直接绑定隐藏真相，会造成“NPC 全知”和判断评分泄题；
- 在 UI 中重新增加结果特判，会破坏纯规则回放和 React/独立 HTML 一致性；
- 免费或无机会成本的送检会成为支配策略；
- 把披露文案直接写成正负概率修改会混淆“事实改变后验”和“表达方式改变关系”两个通道；
- 让动态开价跟随每个微小状态 delta 连续抖动，会削弱玩家对证据因果的理解；正式重估必须保留事件门槛和原因；
- 把实际成交价纳入完全知情最优基准会让任何盈利成交自动满分，结算基准必须独立于玩家本次出价；
- 把当前路径动态形成的最低成交线当作全局最优，也会把“这条路径没有后悔”误写成“整局经营满分”；
- 把已经被否定的规则继续保存在回归测试中，会让测试稳定地保护错误假设；
- 过早扩展第二案件、真实市场数据库或正式美术，会掩盖当前平衡和理解度未知；
- TypeScript 主规则与独立 HTML 客户端目前仍是两份实现，后续必须保留差分测试或统一规则源。
- 若让插画像素承担微观鉴定事实，玩家会被迫猜图；高保真必须把插画定位与权威文字观察分开。

## 当前未知

- 玩家和老师能否清楚理解“客观结果”与“判断质量”共同构成的胜负；
- `D—SSS` 等级阈值、品质上限与三个分项之间如何平衡；
- 独立议价容量如何由 NPC 人物、演绎结果和当前状态共同计算；
- 选择具体证据披露后，不同人物的反馈是否足够清楚；
- 玩家能否清楚区分私有证据、共享证据、NPC 真诚记忆和 NPC 主观判断；
- 6 点预算、送检 `2 AP + 10`、谨慎参考价、动态普通接受线和买断线是否平衡；
- `80 → 65 → 70` 这类事件驱动重估是否被理解为双方认知变化，而不是系统任意改价；
- 无条件买断是否只在急售、风险规避和高不确定性组合下占优，而不是更便宜的普通报价；
- 线索的温和导向是否足够，是否仍存在唯一最优路线或支配策略；
- 普通网页 H5 在最终 HTTPS 托管和微信内浏览器中的兼容、启动与分享表现；
- 后续精品、珍品案件的写实结算讲解图由谁制作、如何进行文化与器物知识校订；
- 首案真实文化内容、器物知识和数值是否需要专家校订。
