...
V1 已在独立分支 `codex/v1-teacher-finalize` 完成教师交付所需的最小收口：正式高保真权威、历史低保真与数值实验台边界已经明确，教师项目/演示说明已经完成，完整门禁达到 `81/81` 并复跑桌面与手机真实浏览器路径，提交前独立 QA 已通过。当前还差最终提交、提交后冻结点 QA、冻结标签和仓库外交付包；原工作区混合改动仍完整保全且未导入。
...
- 当前没有合并、移动 `main`、打冻结标签、制作最终交付包、启动 V2、推送或部署。
...
- V1 最小修正、教师说明和提交前独立 QA 已经完成；最终冻结提交、提交后 QA、标签、教师包、源码包与最终 bundle 尚未完成；V2 尚未启动。

## 当前最大风险

当前正式教师权威已经清楚，完整测试也已全绿；最大风险转为冻结与长期演进边界：

1. 最终提交、tag 与两个交付包尚未生成，当前结果还不是正式冻结引用；
2. 根工作区仍保存历史低保真改动、对齐文档、周报和临时文件，不能未经审阅混入 V1；
3. 当前只有一个简单教学案，无法证明数值平衡、不同案件的信息披露与一般玩家体验；
4. 生产规则、实验台、历史单文件与测试之间的数值来源仍需在 V2 统一；
...
- V1 只剩最终提交、提交后冻结点 QA、冻结 tag、白名单教师包、完整源码包与包体复验；
