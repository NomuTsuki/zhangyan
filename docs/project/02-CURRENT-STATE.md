...
V2 已冻结为可运行、可回放的玩家试玩底座；V3 当前是 **Design / v0.3 作者规格已获用户批准，进入玩家知识工作台微循环设计**。冻结作者合同、具体一局和限定领域映射不再继续扩写；当前用同一段已批准内容比较两至三个玩家侧低保真表达分支。真人测试合同、正式数值与产品代码继续锁定。
...
- [DEC-023](decisions/DEC-023-v3-pre-product-validation-and-player-projection-boundary.md) 已固定正式产品代码前依次通过作者结构／演算、限定领域、用户作者规格、玩家微循环／真人测试、数值模拟、整局原型／真人测试、V2 技术审查和明确开工批准；同时批准系统整理／玩家确认、文字首层／粗数按需、常驻方向／提示按需加深的首轮玩家投影方向。具体 UI、测试合同、正式数值和产品实现仍未批准；
...
- Workstream：`V3-PLAYER-KNOWLEDGE-MICROCYCLE-001`；状态：In progress / Low-fidelity design entry；责任载体：Project Co-Leader（玩家投影设计与阶段交接）；当前检查点：作者规格门已关闭，下一步用同一段已批准内容制作两至三个低保真表达分支，比较事实、当前解释、开放问题、调查方向和按需粗概率。授权边界：允许讨论、记录并制作与产品实现隔离的低保真微循环；不修改正式产品代码，不决定正式概率／价格／成本，不执行真人测试，不推送、部署或发布。preserved surface：作者完整图不局中泄露，系统整理本局已知内容，玩家保留关注、判断、继续与停手责任，提示不发布唯一动作。stop／reopen trigger：分支必须泄露隐藏门才能成立、玩家确认退化为空按钮、需要新产品数值／测试通过线，或原型暴露作者合同新冲突；
...
当前没有新暴露且未回答的作者侧分歧。用户已经批准 v0.3 作为玩家微循环输入；现在的第一个设计前沿是：同一段已知内容怎样在“系统整理”与“玩家作出真实判断”之间分工，而不把知识工作台做成答案卡或无意义确认器。

v0.3 已取得当前冻结输入的独立结构／确定性 Pass，并由 DEC-024 完成用户作者规格门。当前只进入玩家微循环低保真设计；原型形成后仍须按 DEC-023 单独披露并批准无答案真人测试合同，再做数值模拟、整局原型／真人测试、V2 技术审查和用户明确开工批准，才可修改正式产品代码。任何阶段都不把 DEC-018 的 Experimental 数值称为玩法定稿。

## 当前边界

- V3 总范围包含物品真相、证据拓扑、玩家知识图、玩家后验与调查／停止后果；**作者侧责任区已完成当前门，当前工作只推进玩家知识工作台微循环的低保真表达比较，不进入正式 UI 或产品实现**；
