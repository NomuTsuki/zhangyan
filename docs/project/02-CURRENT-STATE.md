...
**本仓库是当前活跃仓库，V3 在此继续。** 知识地图语义已于 2026-08-24 全部闭合（DEC-026 骨架 + DEC-028 地标汇合 + DEC-029 罗盘排序范围），2026-08-25 由 DEC-030 补上“无效调查代价归属”这一条边界；作者规格与首案稳定 G2 边界已通过结构验证。2026-08-25 已产出两份离线走查草图（作者侧全貌 ＋ 玩家完整一局逐步显影），把已批准语义变成可看的对象；**可玩切片尚未开始，是当前唯一在做的事**。
...
当前仓库是迁移后的活跃仓库，正在承接 V3 玩家侧工作。DEC-027 已关闭首案“回退”的内容范围：G2 保持稳定，后段失败表现为局部路线被排除、G3 未形成和成本保留；通用降级仍在引擎防御层。新增范围覆盖层已经只读独立 **Pass**。**当前没有阻断项。** 地标汇合与重排语义已由 DEC-028 依据三个模型的渲染比较定案；罗盘对“当前最优调查”排序范围的作用已由 DEC-029 定案(不限定定义域、严格并列时按罗盘破平、越界必须标注)；无效／重复调查的代价已由 DEC-030 定案(只消耗成本与剩余机会，不下调系统对器物的评价，玩法质量只由判断质量一轨承担)。下一步是按 DEC-026／028／029／030 语义制作桌面尺寸的可丢弃可玩地图切片，见 `03-NEXT-ACTIONS.md` 第 17 条。
