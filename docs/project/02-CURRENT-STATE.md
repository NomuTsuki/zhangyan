...
当前仓库没有待解决的收尾阻断；它已经冻结，不再承接新的设计、原型或实现。DEC-027 已关闭首案“回退”的内容范围：G2 保持稳定，后段失败表现为局部路线被排除、G3 未形成和成本保留；通用降级仍在引擎防御层。新增范围覆盖层已经只读独立 **Pass**。

v0.3 的旧冻结输入已取得独立结构／确定性 Pass 并由 DEC-024 完成作者规格门；DEC-027 不改旧字节，只新增当前范围解释和验证层，该层现已独立 Pass。剩余玩家侧工作从知识地图低保真切片开始，全部转交迁移后的新仓库；之后仍须按 DEC-023 依次处理粗概率、无答案真人测试合同、数值模拟、整局原型／真人测试、V2 技术审查和用户明确开工批准。任何阶段都不把 DEC-018 的 Experimental 数值或“当前最优调查”的未校准排序称为玩法定稿。

## 当前边界

- V3 总范围包含物品真相、证据拓扑、玩家知识图、玩家后验与调查／停止后果；**作者侧责任区已完成当前门，玩家地图核心语义已批准；新版知识地图低保真切片尚未开始，后续只在迁移后的新仓库继续**；
...
- 本次冻结提交、annotated tag 与全 refs 推送完成后，不再在本仓库新增提交或工作树；未执行 GitHub 网页端 Archive，不 deploy、不制作最终 PDF、不公开使用私有批注图。
