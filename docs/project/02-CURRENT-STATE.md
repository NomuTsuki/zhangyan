...
当前仓库是迁移后的活跃仓库，正在承接 V3 玩家侧工作。DEC-027 已关闭首案“回退”的内容范围：G2 保持稳定，后段失败表现为局部路线被排除、G3 未形成和成本保留；通用降级仍在引擎防御层。新增范围覆盖层已经只读独立 **Pass**。当前阻断只有一项：知识地图的地标汇合语义与罗盘排序范围尚未对齐，它们是可玩切片的前置条件。
