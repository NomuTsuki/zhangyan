...
const out=resolve('output/playwright/frontier-system-repair-delivery-2026-09-13/'+(capture?'browser-capture':'browser'));await mkdir(out,{recursive:true});
...
const cases=[
 ['history-source-review',['W','B','P','T2','A2','COR2'],'C','question:history'],
...
A.COR2='A.CORROBORATE.ARCHIVE.T2_CURRENT';A.REPEAT='A.COMPARE.CORPUS.RECHECK';A.CMP_ARCHIVE=A.CMP;
try{for(const [name,pre,step,qid] of cases){
 ...
 try{await page.goto(url);await page.waitForFunction(()=>window.hifiSnapshot?.().engineReady&&window.hifiSnapshot().visibleBody.includes('OBJ_R'));for(const id of pre)await take(A[id]);await page.keyboard.press('Escape');await page.locator('.map-fit').click();
  ...
  const persistent=['history-source-review','uv-to-regional-synthesis','alternative-identity-route','visible-repairs-branch','record-to-object-endpoint'].includes(name)||name.startsWith('handover-interrupt');
  ...
  if(name==='history-source-review'){
    const q=after.app.graph.frontiers.find(f=>f.id===qid);assert.ok(q.sourceIds.includes('obs.phase.t1'));assert.ok(q.sourceIds.includes('obs.object.continuity'));
    await page.locator('[data-map-frontier="question:history"]').click();
    await page.getByRole('button',{name:/旧照片中的锔修痕迹/}).last().click();
    assert.equal(await page.locator('.material-reader').getAttribute('data-observation-id'),'obs.phase.t1');
    await page.screenshot({path:resolve(out,name+'-evidence.png')});
  }
