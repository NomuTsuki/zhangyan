...
const out='output/playwright/frontier-system-audit-2026-09-13';await mkdir(out,{recursive:true});
const file='docs/project/evidence/v3-design/validation/workbench-hifi-v0/prototype.html';
const report={buildSha256:createHash('sha256').update(await readFile(file)).digest('hex'),readOnly:true,orders:0,transitions:0,uniqueTransitions:0,frontiers:{},retirements:[],directionMismatches:[]};
...
report.summary={frontierIds:Object.keys(report.frontiers).length,retirementVariants:report.retirements.length,immediateVariants:report.retirements.filter(r=>r.retireAt===0).length,misdirectedVariants:report.directionMismatches.length};
