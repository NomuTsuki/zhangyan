 ...
 * Positions are centers; old positions are retained unless a resize/collision
 * makes that impossible. No author topology, topic lane or kind rank is used.
 */
...
function choosePosition(node, positions, neighbours, graphEdges, width) {
  ...
  const desired = connected.length
...
export function layoutGraph(graph, { width = 960, height = 680, previous = null } = {}) {
  width = Math.max(400, Number.isFinite(width) ? width : 960);
  ...
  const positions = {}, movements = [], relocated = [], adjacency = new Map(nodes.map(node => [node.id, []]));
  ...
  for (const node of nodes) {
    ...
    if (old && Number.isFinite(old.x) && Number.isFinite(old.y)) {
      const candidate = nodePosition(node, clamp(old.x, MARGIN + NODE_WIDTH / 2, width - MARGIN - NODE_WIDTH / 2), Math.max(MARGIN + NODE_HEIGHT / 2, old.y));
      if (!Object.values(positions).some(position => boxesOverlap(candidate, position, GAP))) positions[node.id] = candidate;
      else { pending.push(node); relocated.push(node.id); }
    } else pending.push(node);
  }
  while (pending.length) {
    // Attached additions grow first; a disconnected acquired island is legal.
    let nextIndex = 0, mostPlaced = -1;
    for (let index = 0; index < pending.length; index += 1) {
      const count = adjacency.get(pending[index].id).filter(id => positions[id]).length;
      if (count > mostPlaced) { mostPlaced = count; nextIndex = index; }
    }
    const [node] = pending.splice(nextIndex, 1);
    positions[node.id] = choosePosition(node, positions, adjacency.get(node.id), edges, width);
  }
  const overlaps = [], routeNodeIntersections = [], entries = Object.entries(positions);
  for (let i = 0; i < entries.length; i += 1) {
    const [id, position] = entries[i], old = oldPositions[id];
    if (old && Number.isFinite(old.x) && Number.isFinite(old.y)) movements.push({ id, distance: round(distance(old, position)), from: { x: old.x, y: old.y }, to: { x: position.x, y: position.y } });
  ...
  const routedEdges = edges.map(edge => {
    ...
    const obstacles = routeObstacles(positions, edge);
    const points = insetRoad(route({ x: from.anchorX, y: from.anchorY }, { x: to.anchorX, y: to.anchorY }, obstacles));
    ...
    const label = edgeLabel(points, positions);
    return { ...edge, path: toPath(points), points, labelX: round(label.x), labelY: round(label.y) };
  });
  const bottom = Math.max(height, ...entries.map(([, position]) => position.y + NODE_HEIGHT / 2 + MARGIN));
  ...
  return { positions, edges: routedEdges, width, height: Math.ceil(bottom), diagnostics: {
    ...
    algorithm: 'experimental-incremental-known-neighbour-growth', nodeWidth: NODE_WIDTH, nodeHeight: NODE_HEIGHT,
    nodeBoxAreaRatio: round(entries.length * NODE_WIDTH * NODE_HEIGHT / (width * Math.ceil(bottom))),
    averageRoadLength: round(lengths.length ? lengths.reduce((sum, length) => sum + length, 0) / lengths.length : 0),
    maxRoadLength: round(Math.max(0, ...lengths)),
    roadEndpointInsets: { start: ROAD_START_INSET, end: ROAD_END_INSET },
    labelPolicy: 'edge labels on local focus only; label-label collisions not optimized',
  } };
}
