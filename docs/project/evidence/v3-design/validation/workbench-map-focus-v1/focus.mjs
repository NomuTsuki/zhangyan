...
function focusSeedNodeIds(body, focus) {
...
export function focusForTopic(topicId, graph) {
  const body = graphBody(graph);
  const topic = typeof topicId === "string"
    ? (body.topics ?? []).find((candidate) => candidate.id === topicId)
    : topicId;
  if (!topic?.id || !Array.isArray(topic.memberIds)) return null;
  const members = new Set(topic.memberIds);
  const actionIds = (body.nodes ?? [])
    .filter((node) => members.has(node.id) && node.actionId)
    .map((node) => node.actionId);
  return makeFocus(actionIds, "topic", {
    topicId: topic.id,
    observationIds: freezeList(topic.memberIds),
  });
}

export function relatedTopicIds(graph, focus) {
  const body = graphBody(graph);
  const seeds = focusSeedNodeIds(body, focus);
  const ids = new Set();
  if (focus?.topicId) ids.add(focus.topicId);
  for (const topic of body.topics ?? []) {
    if (topic.memberIds.some((id) => seeds.has(id))) ids.add(topic.id);
  }
  return ids;
}
...
export function relatedNodeIds(graph, focus) {
  ...
  const ids = focusSeedNodeIds(body, focus);

  // One hop shows why the selected evidence matters without turning a precise
  // action click into a highlight of the whole long-term question.  Topic
  // membership is also one hop, but topics are grouping metadata rather than
  // graph nodes.  Both expansions read the original seed snapshot so a bridge
  // observation cannot fan the focus into a second topic or a second edge.
  const seeds = new Set(ids);
  for (const topic of body.topics ?? []) {
    if (!topic.memberIds.some((id) => seeds.has(id))) continue;
    for (const id of topic.memberIds) if (knownIds.has(id)) ids.add(id);
  }
