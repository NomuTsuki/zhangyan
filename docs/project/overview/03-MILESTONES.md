...
flowchart LR
    ...
    Approve --> Micro["当时下一步<br/>微循环低保真分支<br/>(后于 08-22 否决)"]
    Micro --> Contract["当时之后<br/>无答案测试合同"]
```
...
- **现在：** 旧原型已删除并保留失败证据；项目级 UI／UX Skills 已按固定来源安装、核验并建立可退出 Git 检查点。用户逐项确认地点／两态道路／地标／足迹／罗盘、纯黑与显影、竞争路线、问题级探索前沿和逐级帮助；DEC-026 又确认最强按需层点名当前最优调查。随后 DEC-027 固定首案 G2 地标稳定、局部路线可失败、G3 可不形成，并把永久 G2 降级场景收窄为通用防御／未来案件测试；旧 `40/40` 六文件不改写，新增范围覆盖层已取得只读独立 Pass。
- **保持不变／尚未验证：** 作者完整图、G1—G3、开放调查、DEC-025 玩家责任、手动停手、三轨经济和产品代码开工门不变；新版低保真表达、粗概率、行动排序权重／并列规则、玩家理解、趣味、正式 UI 与代码都尚未成立。
- **下一道交接门：** 对齐阶段地标成立时地图怎样汇合与重排；随后才制作隔离的新版地图微循环并提交无答案真人测试合同。

```mermaid
flowchart LR
    Old["文字／面板工作台<br/>已否决并删除"] --> Semantics["渐进地图核心语义<br/>DEC-026 已完成"]
    Semantics --> StableG2["稳定 G2 与范围分界<br/>DEC-027 已批准"]
    StableG2 --> Landmark["当时下一步<br/>剩余地标变化语义<br/>(08-24 由 DEC-028/029 定案)"]
    Landmark --> Prototype["之后<br/>新版地图微循环"]
    Prototype --> Contract["再之后<br/>无答案测试合同"]
```

### 2026-08-24 · 地图语义闭合，工作从"讨论表达"转为"做出可玩的东西"

- **为什么做：** V3 自 2026-08-13 起产出 28 份设计文档约 4,754 行，可玩产物为 0；唯一一次玩家侧原型被否决删除。继续在纸上争论表达问题的边际价值已经很低——这也是"整齐但平庸"这一判断的量化依据。
- **之前：** DEC-026 的地图骨架已批准，但"地标怎么汇合、重排、被推翻后去哪里"和"罗盘是否限定最强提示的排序范围"两项语义未决，被列为切片的前置条件；三个语义问题原本要求全部纸上对齐后才允许做原型。
- **现在：** 三个候选地标模型各做静态线框草图并**实际渲染比较**，[DEC-028](../decisions/DEC-028-v3-landmarks-at-evidence-junctions-and-compressed-validation-order.md) 采用"画在证据道路汇合口"，模型二（G 降为图外读数）与模型三（可折叠结论层）由渲染证据否决——纸上曾预判汇合口"天然拥挤"，渲染后在桌面宽度下并不拥挤，**该预判失准，说明这一类问题应当先渲染再判断**。[DEC-029](../decisions/DEC-029-v3-compass-does-not-restrict-best-investigation-ranking.md) 采用"罗盘不限定排序定义域、只在严格并列时破平、越界必须标注"。粗概率呈现形式与"越界时给一条还是两条"由前置条件降为切片内变体验证。视口按用户 2026-08-24 决定改为**电脑浏览器尺寸**，手机竖屏约束搁置到高保真阶段重议。
- **保持不变／尚未验证：** 冻结作者六文件与哈希、DEC-018 Experimental 数值、三档结果语义、手动停手、三轨经济与产品代码开工门全部不变；**地图的表达是否被无答案玩家正确理解、是否有能动性、是否好玩，全部未验证**，只能由无答案真人测试举证。渲染证据只能证明"读得出来"，不能证明"好玩"。
- **下一道交接门：** 做出按桌面尺寸、允许整体丢弃的可玩地图切片，再提交无答案真人测试合同。

```mermaid
flowchart LR
    Sketch["三模型静态草图<br/>渲染比较"] --> Landmark2["地标画在汇合口<br/>DEC-028"]
    Landmark2 --> Compass2["罗盘不限定排序<br/>DEC-029"]
    Compass2 --> Slice["当前<br/>桌面尺寸可玩切片"]
    Slice --> Contract2["之后<br/>无答案测试合同"]
```
