...
|---|---:|
| 初始调查行动点 | 6 |
| 初始议价容量 | 由 NPC 当前状态计算（默认 NPC 当前为 5） |
| 卖家开价 | 80 |
...
议价容量：
每次正式报价或折价（含无条件买断报价） = 1
NPC 对该次报价的同轮还价 = 0
接受当前价格 = 0
拒绝 = 0
```

每类有成本动作先校验自己的资源：调查动作校验剩余调查行动点，正式报价或折价校验剩余议价容量；折价不消耗调查行动点。NPC 对同一次报价的还价不重复扣议价容量。即使议价容量已经为 `0`，玩家仍可接受当前价格或拒绝交易完成终局；购买、拒绝和查看也不要求剩余调查行动点。
